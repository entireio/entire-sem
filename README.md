...
- **Profiles.** Search defaults to the `fast` profile so the indexed pool carries high-precision call edges: caller-degree ranking and graph expansion need them to lift an implementation above same-named test scaffolding. `--profile syntax-only` drops relation extraction for the lowest-latency lexical-only search; `--profile full` adds every relation family when deeper semantics are worth the cost.
